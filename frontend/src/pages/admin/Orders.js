...
//   const [playBeep] = useSound(beepSound);
  ...
  const fetchStats = async () => {
    try {
      const response = await fetch('https://happyfamilyrestaurant.onrender.com/api/orders/stats', {
        headers: {
          'Authorization': `Bearer ${localStorage.getItem('token')}`
        }
      });
  ...
  const updateOrderStatus = async (orderId, status) => {
    try {
      const response = await fetch(`https://happyfamilyrestaurant.onrender.com/api/orders/${orderId}/status`, {
        method: 'PUT',
        headers: {
