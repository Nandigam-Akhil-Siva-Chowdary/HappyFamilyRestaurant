...
const AdminDashboard = () => {
  ...
  const fetchDashboardStats = async () => {
    try {
      ...
      // Fetch orders stats
      const ordersResponse = await fetch('http://localhost:5000/api/orders/stats', {
        headers: { 'Authorization': `Bearer ${token}` }
      });
      const ordersStats = ordersResponse.ok ? await ordersResponse.json() : {};
      
      // Fetch menu stats
      const menuResponse = await fetch('http://localhost:5000/api/menu/stats/count', {
        headers: { 'Authorization': `Bearer ${token}` }
      });
      ...
      const contactResponse = await fetch('http://localhost:5000/api/contact/stats', {
        headers: { 'Authorization': `Bearer ${token}` }
      });
